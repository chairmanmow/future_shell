...
function JSONChat(usernum,jsonclient,host,port) {
	...
	this.join = function(target) {
		...
		if(msgcount == 0) {
			...
			var str = format("\1h\1m-Last msg: %.2d:%.2d on %.2d/%.2d \r\n",
	...
	this.who = function(target) {
		...
		chan.messages.push(new Message(undefined,"\1h\1c+Users in \1w" + chan.name + "\1w:\1g " + uList.join("\1h\1y-\1h\1g") + " \r\n"));
		return chan.users;
	}
	...
	this.update = function(packet) {
		...
		case "SUBSCRIBE":
			channel.messages.push(new Message(system.timestr() + "\1h\1g\r\n\r\n",packet.data.nick + "\1g is here.\r\n",Date.now()));
			channel.users = getUserList(this,channel);
			break;
		case "UNSUBSCRIBE":
			channel.messages.push(new Message("",packet.data.nick + " has left.\r\n",Date.now()));
			channel.users = getUserList(this,channel);
			break;
		case "WRITE":
	...
	this.action = function(target,action) {
		var message = new Message(undefined,"\1h\1g " + this.nick.name + "\1h\1g " + action + "\r\n");
		var chan = this.channels[target.toUpperCase()];
		this.client.write("chat","channels." + chan.name + ".messages",message,2);
		this.client.push("chat","channels." + chan.name + ".history",message,2);
		chan.messages.push(message);
