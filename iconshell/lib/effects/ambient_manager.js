...
function ShellAmbientManager(frame, opts){
    this.frame = frame; this.opts = opts||{};
    this.animations = {}; this.order=[]; this.current=null; this.prevName=null;
    this.lastSwitch = time();
    this.switchInterval = (this.opts.switch_interval||90); // seconds idle between switches while running
    this.running = false;
    this.fps = Math.max(1, this.opts.fps||8);
    this._tickAccum = 0; // for throttling if caller cycles faster
    this.sequenceIndex = 0; // for deterministic sequence mode
    this._timer = null; this._timerEvent = null; // external timer attachment
}
ShellAmbientManager.prototype.add = function(name, ctor){ if(!this.animations[name]){ this.animations[name]=ctor; this.order.push(name); } };
ShellAmbientManager.prototype._pickNext = function(){
    if(!this.order.length) return null;
    // Deterministic sequence if random flag false and sequence provided in opts
...
ShellAmbientManager.prototype.start = function(name){
    ...
    if(this.current && this.current.dispose){ try{ this.current.dispose(); }catch(e){} }
    if(!name) name=this._pickNext();
    var ctor=this.animations[name]; if(!ctor) return;
    this.current=new ctor();
    try{ this.current.init(this.frame, this.opts); }catch(e){ try{ log('ambient init error '+name+': '+e); }catch(_){}; this.current=null; return; }
    if(this.opts.clear_on_switch){ try{ this.frame.clear(); }catch(e){} }
    this.prevName=name; this.lastSwitch=time(); this.running=true;
...
ShellAmbientManager.prototype.stop = function(){
    if(this.current && this.current.dispose){ try{ this.current.dispose(); }catch(e){} }
    if(this.current){ try{ log('ambient stop '+(this.prevName||'?')); }catch(_){ } }
...
ShellAmbientManager.prototype.cycle = function(){
    ...
    // Switch if interval elapsed
    if(this.switchInterval>0 && time() - this.lastSwitch >= this.switchInterval){ this.start(); }
    try{ this.current.tick(); }catch(e){ try{ log('ambient tick error '+e); }catch(_){} }
};
ShellAmbientManager.prototype.dispose = function(){ this.stop(); };

// Attach external Timer (Synchronet event-timer) for autonomous cycling instead of manual main loop calls.
...
ShellAmbientManager.prototype.detachTimer = function(){
    if(this._timerEvent){ try{ this._timerEvent.abort = true; }catch(e){} this._timerEvent=null; }
    this._timer=null;
};
